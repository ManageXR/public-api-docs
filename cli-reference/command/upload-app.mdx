...
```
## Examples
### 1. upload-app using JSON file auth
```
mxr-cli upload-app /path/to/apkfile.apk --api-key-file /path/to/api_key.json
```
### 2. upload-app using environment variable auth
```
MXR_API_KEY_ID="xxx" MXR_API_KEY_SECRET="xxx" mxr-cli upload-app /path/to/apkfile.apk
```
### 2. upload-app using command options
In this example, we include an obb with the application
